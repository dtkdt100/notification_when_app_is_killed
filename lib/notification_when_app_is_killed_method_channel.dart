...
class MethodChannelNotificationWhenAppIsKilled extends NotificationWhenAppIsKilledPlatform {
  /// The method channel used to interact with the native platform.
  @visibleForTesting
  final methodChannel = const MethodChannel('notification_when_app_is_killed');

  @override
  Future<bool?> setNotificationOnKillService(ArgsForKillNotification argsForKillNotification) async {
    final success =
        await methodChannel.invokeMethod<bool>('setNotificationOnKillService', argsForKillNotification.toJson());
  ...
  Future<bool?> cancelNotificationOnKillService() async {
    final success = await methodChannel.invokeMethod<bool>('cancelNotificationOnKillService');
